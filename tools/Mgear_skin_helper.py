"""
This tool is designed to work with the default Mgear rig template controls naming convension,
i use it while skinning a biped, it is designed to save a bit of time by adding animation to the arms, head, etc
so i can see deformations without needing to move the controls myself, it also allows you to save out poses and import 
poses, cycle through saved poses quickly, the json output file will be saved to a directory called PipeDreams(my pipeline name) in the users Documents.

it only works with T-posed mgear bipeds at real world scale 1.7m and its still a WIP so not everything works yet, like the fingers =).

- the very top group of the rig needs to have a "_rig" at the end of its naming for the tool to pick it up.
